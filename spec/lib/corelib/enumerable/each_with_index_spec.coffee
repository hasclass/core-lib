describe "Enumerable#each_with_index", ->
  ...
  beforeEach ->
    @b = EnumerableSpecs.NumerousLiteral.new(2, 5, 3, 6, 1, 4)

  it "passes each element and its index to block", ->
    a = R []
    @b.each_with_index( (o, i) -> a.push [o, i] )
    expect(a.valueOf()).toEqual [[2, 0], [5, 1], [3, 2], [6, 3], [1, 4], [4, 5]]
  ...
  it "provides each element to the block and its index", ->
    acc = R []
    res = @b.each_with_index( (a,i) -> acc.push [a, i] )
    expect( acc.valueOf() ).toEqual [[2, 0], [5, 1], [3, 2], [6, 3], [1, 4], [4, 5]]
  ...
  it "binds splat arguments properly", ->
    ...
    res = @b.each_with_index (b...) ->
      ...
      acc.push(c)
      acc.push(d)
    expect( acc ).toEqual [2, 0, 5, 1, 3, 2, 6, 3, 1, 4, 4, 5]
    expect( res).toEqual @b
  ...
  xdescribe "ruby_version_is '1.8.7'", ->
    it "returns an enumerator if no block", ->
      ...
      expect( e.to_a().valueOf() ).toEqual [[2, 0], [5, 1], [3, 2], [6, 3], [1, 4], [4, 5]]
