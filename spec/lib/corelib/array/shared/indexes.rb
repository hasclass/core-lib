describe :array_indexes, :shared => true do
  it "returns elements at integer argument indexes (DEPRECATED)", ->
    array = [1, 2, 3, 4, 5]
  ...
  it "tries to convert the passed arguments to Integers using #to_int", ->
    obj = mock('to_int')
    obj.should_receive(:to_int).and_return(1, 3)
    [1, 2, 3, 4, 5].send(@method, obj, obj).should == [2, 4]

  it "returns elements in range arguments as nested arrays (DEPRECATED)", ->
    array = [1, 2, 3, 4, 5]
    params = [0..2, 1...3, 4..6]
