...
describe "Array#pack with format 'M'", ->
  ...
  it "encodes nil as an empty string", ->
    [nil].pack("M").should == ""

  it "appends a soft line break at the end of an encoded string", ->
    ["a"].pack("M").should == "a=\n"

  it "does not append a soft break if the string ends with a newline", ->
    ["a\n"].pack("M").should == "a\n"

  it "encodes one element for each directive", ->
    ["a", "b", "c"].pack("MM").should == "a=\nb=\n"

  it "encodes byte values 33..60 directly", ->
    [ [["!\"\#$%&'()*+,-./"], "!\"\#$%&'()*+,-./=\n"],
      [["0123456789"],        "0123456789=\n"],
      [[":;<"],               ":;<=\n"]
    ].should be_computed_by(:pack, "M")

  it "encodes byte values 62..126 directly", ->
    [ [[">?@"],                         ">?@=\n"],
  ...
  it "encodes an '=' character in hex format", ->
    ["="].pack("M").should == "=3D=\n"

  it "encodes an embedded space directly", ->
    ["a b"].pack("M").should == "a b=\n"

  it "encodes a space at the end of the string directly", ->
    ["a "].pack("M").should == "a =\n"

  it "encodes an embedded tab directly", ->
    ["a\tb"].pack("M").should == "a\tb=\n"

  it "encodes a tab at the end of the string directly", ->
    ["a\t"].pack("M").should == "a\t=\n"

  it "encodes an embedded newline directly", ->
    ["a\nb"].pack("M").should == "a\nb=\n"

  it "encodes 0..31 except tab and newline in hex format", ->
  ...
  it "encodes a tab followed by a newline with an encoded newline", ->
    ["\t\n"].pack("M").should == "\t=\n\n"

  it "encodes 127..255 in hex format", ->
  ...
  it "emits a soft line break when the output exceeds 72 characters when passed '*', 0, 1, or no count modifier", ->
  ...
  it "emits a soft line break when the output exceeds count characters", ->
    [ [["abcdefghi"],        "M2", "abc=\ndef=\nghi=\n"],
      [["abcdefghi"],        "M3", "abcd=\nefgh=\ni=\n"],
      [["abcdefghi"],        "M4", "abcde=\nfghi=\n"],
      [["abcdefghi"],        "M5", "abcdef=\nghi=\n"],
  ...
  it "calls #to_s to convert an object to a String", ->
    obj = mock("pack M string")
    obj.should_receive(:to_s).and_return("packing")

    [obj].pack("M").should == "packing=\n"

  it "converts the object to a String representation if #to_s does not return a String", ->
    obj = mock("pack M non-string")
    obj.should_receive(:to_s).and_return(2)

    [obj].pack("M").should be_an_instance_of(String)

  it "encodes a Symbol as a String", ->
    [:symbol].pack("M").should == "symbol=\n"

  it "encodes an Integer as a String", ->
    [ [[1],             "1=\n"],
      [[bignum_value],  "#{bignum_value}=\n"]
    ].should be_computed_by(:pack, "M")

  it "encodes a Float as a String", ->
    [1.0].pack("M").should == "1.0=\n"

  ruby_bug "#3273", "1.8.7", ->
    it "converts Floats to the minimum unique representation", ->
      [1.0 / 3.0].pack("M").should == "0.3333333333333333=\n"
  
  ruby_version_is "".."1.9", ->
    describe "with a multibyte $KCODE", ->
      before :each do
        @kcode = $KCODE
  ...
  ruby_version_is "1.9", ->
    it "sets the output string to US-ASCII encoding", ->
...
describe "Array#pack with format 'm'", ->
  ...
  it "appends a newline to the end of the encoded string", ->
    ["a"].pack("m").should == "YQ==\n"

  it "encodes one element per directive", ->
    ["abc", "DEF"].pack("mm").should == "YWJj\nREVG\n"

  it "encodes 1, 2, or 3 characters in 4 output characters (Base64 encoding)", ->
    [ [["a"],       "YQ==\n"],
      [["ab"],      "YWI=\n"],
      [["abc"],     "YWJj\n"],
      [["abcd"],    "YWJjZA==\n"],
  ...
  it "emits a newline after complete groups of count / 3 input characters when passed a count modifier", ->
    ["abcdefg"].pack("m3").should == "YWJj\nZGVm\nZw==\n"

  it "implicitly has a count of 45 when passed '*', 1, 2 or no count modifier", ->
  ...
  it "encodes all ascii characters", ->
  ...
  it "raises a TypeError if #to_str does not return a String", ->
    obj = mock("pack m non-string")
    expect( -> [obj].pack("m") ).toThrow(TypeError)

  it "raises a TypeError if passed nil", ->
    expect( -> [nil].pack("m") ).toThrow(TypeError)

  it "raises a TypeError if passed an Integer", ->
    expect( -> [0].pack("m") ).toThrow(TypeError)
    expect( -> [bignum_value].pack("m") ).toThrow(TypeError)

  ruby_version_is ""..."1.9", ->
    it "emits a newline after 45 characters if passed zero as the count modifier", ->
      s = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
      r = "YWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFh\nYWFhYWE=\n"
      [s].pack("m0").should == r
  
  ruby_version_is "1.9", ->
    it "does not emit a newline if passed zero as the count modifier", ->
      s = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
