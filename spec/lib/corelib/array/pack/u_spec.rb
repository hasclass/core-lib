...
describe "Array#pack with format 'U'", ->
...
describe "Array#pack with format 'u'", ->
  ...
  it "prepends the length of each segment of the input string as the first character (+32) in each line of the output", ->
    ["abcdefghijklm"].pack("u7").should == "&86)C9&5F\n&9VAI:FML\n!;0``\n"

  it "encodes 1, 2, or 3 characters in 4 output characters (uuencoding)", ->
    [ [["a"],       "!80``\n"],
      [["ab"],      "\"86(`\n"],
      [["abc"],     "#86)C\n"],
      [["abcd"],    "$86)C9```\n"],
  ...
  it "implicitly has a count of 45 when passed '*', 0, 1, 2 or no count modifier", ->
  ...
  it "raises a TypeError if passed nil", ->
    expect( -> [nil].pack("u") ).toThrow(TypeError)

  it "raises a TypeError if passed an Integer", ->
    expect( -> [0].pack("u") ).toThrow(TypeError)
    expect( -> [bignum_value].pack("u") ).toThrow(TypeError)
