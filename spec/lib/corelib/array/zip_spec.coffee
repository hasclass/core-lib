describe "Array#zip", ->
  it "returns an array of arrays containing corresponding elements of each array", ->
    expect( R([1, 2, 3, 4]).zip(["a", "b", "c", "d", "e"]).unbox(true) ).toEqual [[1, "a"], [2, "b"], [3, "c"], [4, "d"]]

  it "fills in missing values with nil", ->
    expect( R([1, 2, 3, 4, 5]).zip(["a", "b", "c", "d"]).unbox(true) ).toEqual [[1, "a"], [2, "b"], [3, "c"], [4, "d"], [5, null]]

  it "does not get confused with false", ->
    expect( R([1, 2, 3, 4, 5]).zip(["a", "b", "c", false]).unbox(true) ).toEqual [[1, "a"], [2, "b"], [3, "c"], [4, false], [5, null]]
  ...
  it "calls #to_ary to convert the argument to an Array", ->
    obj =
      to_ary: -> R([3, 4])
    expect( R([1, 2]).zip(obj).unbox(true) ).toEqual [[1, 3], [2, 4]]
  ...
  it "calls block if supplied", ->
    ...
    expect( values.unbox(true) ).toEqual [[1, "a"], [2, "b"], [3, "c"], [4, "d"]]
