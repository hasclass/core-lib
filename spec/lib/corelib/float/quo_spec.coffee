describe "Float#quo", ->
  it "performs floating-point division between self and a Fixnum", ->
    expect( R.$Float(8.9).quo(7).unbox() ).toBeCloseTo 1.2714285714285716, 0.0001

  xit "performs floating-point division between self and a Bignum", ->
    # expect( R.$Float(8.9).quo(9999999999999 ** 9).unbox() ).toBeCloseTo 8.900000000008011e-117, 0.0001

  it "performs floating-point division between self and a Float", ->
    expect( R.$Float(2827.22).quo(872.111111).unbox() ).toBeCloseTo 3.2418116961704433, 0.0001
  ...
  it "returns 0.0 when the argument is Infinity", ->
    expect( R.$Float(47292.2821).quo(1.0/0.0).unbox() ).toBeCloseTo 0.0, 0.0001

  it "returns -0.0 when the argument is -Infinity", ->
    expect( R.$Float(1.9999918).quo(-1.0/0.0).unbox() ).toBeCloseTo -0.0, 0.0001

  xit "performs floating-point division between self and a Rational", ->
    # expect( R.$Float(74620.09).quo(Rational(2,3)).unbox() ).toBeCloseTo 111930.135, 0.0001

  xit "performs floating-point division between self and a Complex", ->
    # expect( R.$Float(74620.09).quo(Complex(8,2)).unbox() ).toBeCloseTo Complex(, 0.0001
