...
#describe "Float#modulo", ->
  it "returns self modulo other", ->
    expect( R(6543.21).to_f().modulo(137).unbox()    ).toBeCloseTo(104.21, 0.00001)
    expect( R(6543.21).to_f().modulo(137.24).unbox() ).toBeCloseTo(92.9299999999996, 0.00001)
    expect( R(-1.0).to_f().modulo(1).unbox()         ).toEqual 0

  xit 'unsupported', ->
    # expect( R(5667.19).to_f().modulo(bignum_value).unbox() ).toBeClose(5667.19, 0.00001)

  it "returns self when modulus is +Infinity", ->
    expect( R(4.2).to_f().modulo(1/0.0).unbox() ).toEqual 4.2
  ...
  it "returns -0.0 when called on -0.0 with a non zero modulus", ->
    ...
    # expect( r.unbox() ).toEqual 0
    # expect( 1 / r.unbox() ).should < 0
