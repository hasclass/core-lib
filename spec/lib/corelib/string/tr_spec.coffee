describe "String#tr", ->
  it "returns a new string with the characters from from_string replaced by the ones in to_string", ->
    expect( R("hello").tr('aeiou', '*')  ).toEqual R("h*ll*")
    expect( R("hello").tr('el', 'ip')    ).toEqual R("hippo")
    expect( R("Lisp").tr("Lisp", "Ruby") ).toEqual R("Ruby")

  it "accepts c1-c2 notation to denote ranges of characters", ->
  ...
  it "pads to_str with its last char if it is shorter than from_string", ->
    expect( R("this").tr("this", "x")    ).toEqual R("xxxx")
    expect( R("hello").tr("a-z", "A-H.") ).toEqual R("HE...")

  describe "ruby_version_is '1.9.2'", ->
    it "raises a ArgumentError a descending range in the replacement as containing just the start character", ->
      expect( -> R("hello").tr("a-y", "z-b") ).toThrow('ArgumentError')

    it "raises a ArgumentError a descending range in the source as empty", ->
      expect( -> R("hello").tr("l-a", "z") ).toThrow('ArgumentError')

  it "translates chars not in from_string when it starts with a ^", ->
  ...
  it "supports non-injective replacements", ->
    expect( R("hello").tr("helo", "1212") ).toEqual R("12112")

  it "tries to convert from_str and to_str to strings using to_str", ->
    from_str =
      to_str: -> R("ab")
  ...
  #   it "can replace a 7-bit ASCII character with a multibyte one", ->
  #     a = "uber"
...
describe "String#tr!", ->
  ...
  #   it "treats a descending range in the replacement as containing just the start character", ->
  #     expect( R("hello").tr("a-y", "z-b") ).toEqual "zzzzz"
  #   it "treats a descending range in the source as empty", ->
