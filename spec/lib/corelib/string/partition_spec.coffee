  describe "String#partition with String", ->
    it "returns an array of substrings based on splitting on the given string", ->
      expect( R("hello world").partition("o").unbox(true) ).toEqual ["hell", "o", " world"]

    it "always returns 3 elements", ->
      expect( R("hello").partition("x").unbox(true)     ).toEqual(["hello", "", ""])
      expect( R("hello").partition("hello").unbox(true) ).toEqual(["", "hello", ""])

    xit "accepts regexp", ->
      expect( R("hello!").partition(/l./).unbox(true) ).toEqual(["he", "ll", "o!"])
    ...
    describe "ruby_bug redmine #1510, '1.9.1'", ->
      it "converts its argument using :to_str", ->
        find =
          to_str: -> R("l")
        expect( R("hello").partition(find).unbox(true) ).toEqual(["he","l","lo"])
    ...
    it "takes precedence over a given block", ->
      expect( R("hello world").partition("o", -> true ).unbox(true) ).toEqual(["hell", "o", " world"])
