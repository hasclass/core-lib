describe 'ruby_version_is "1.8.7"', ->
  describe "Range#max", ->
    it "returns the maximum value in the range when called with no arguments", ->
      expect( R.Range.new(1,10          ).max() ).toEqual R(10)
      expect( R.Range.new(1,10, true    ).max() ).toEqual R(9)
      expect( R.Range.new('f','l'       ).max() ).toEqual R('l')
      expect( R.Range.new('a','f', true ).max() ).toEqual R('e')

    describe '"1.9"', ->
      it "returns the maximum value in the Float range when called with no arguments", ->
        expect( R.Range.new(303.20,908.1111).max() ).toEqual R(908.1111)
  ...
  describe "Range#max given a block", ->
    ...
    it "passes each pair of elements to the block in reversed order", ->
      acc = []
      R.Range.new(1, 5).max (a,b) -> acc.push [a.to_native(),b.to_native()]; a
    ...
    it "returns the element the block determines to be the maximum", ->
      expect( R.Range.new(1, 3).max (a,b) -> -3 ).toEqual R(1)
